...
async def cb_stock_page(call: types.CallbackQuery, container: Container):
    try:
        ...
        await call.message.edit_text(
            "📦 OMBORDAGI MAHSULOTLAR RO'YXATI:", reply_markup=products_list_kb(products, page=page)
        )
    except Exception:
    ...
    try:
        ...
        if not categories:
            await message.answer(
                "Kategoriyalar hozircha yo'q. Ularni qo'shishingiz mumkin.", reply_markup=categories_list_kb([])
            )
        else:
...
async def process_add_category_name(message: types.Message, state: FSMContext, container: Container):
    try:
        ...
        try:
            ...
            await message.answer(
                f"✅ '{category_name}' kategoriyasi muvaffaqiyatli yaratildi!", reply_markup=main_admin_kb()
            )
...
async def cb_category_page(call: types.CallbackQuery, container: Container):
    try:
        ...
        await call.message.edit_text(
            text, reply_markup=categories_list_kb(categories, for_selection=for_selection, page=page)
        )
    except Exception:
...
async def cb_manage_category(call: types.CallbackQuery, container: Container):
    try:
        ...
        text = f"🗂 <b>{html_mod.escape(cat.name)}</b>\nMahsulotlar: <b>{n}</b> ta\n\nAmalni tanlang:"
...
async def process_category_rename(message: types.Message, state: FSMContext, container: Container):
    try:
        ...
        if updated:
            await message.answer(
                f"✅ Kategoriya nomi yangilandi: <b>{name}</b>", parse_mode="HTML", reply_markup=main_admin_kb()
            )
        else:
...
async def start_add_product(call: types.CallbackQuery, state: FSMContext, container: Container):
    try:
        ...
        await call.message.edit_text(
            "Yangi mahsulot uchun kategoriya tanlang:", reply_markup=categories_list_kb(categories, for_selection=True)
        )
    except Exception:
    ...
    try:
        ...
        try:
            ...
            await product_service.create_product(
                name=data["name"], price=data["price"], quantity=qty, category_id=cat_id
            )
            await message.answer(f"✅ '{data['name']}' mahsuloti qo'shildi!", reply_markup=main_admin_kb())
        except Exception:
            await message.answer(
                "❌ MBga qo'shishda xatolik. Ehtimol bunday nomdagi mahsulot mavjud.", reply_markup=main_admin_kb()
            )
...
async def cb_delete_product_conf(call: types.CallbackQuery, container: Container):
    try:
        ...
        if product:
            await call.message.edit_text(
                f"⚠️ <b>{product.name}</b> mahsulotini o'chirishga ishonchingiz komilmi?\nBu amalni ortga qaytarib bo'lmaydi.",
                parse_mode="HTML",
                reply_markup=product_delete_confirm_kb(product_id),
            )
    except Exception:
...
async def cb_approve_user(call: types.CallbackQuery, container: Container):
    try:
        ...
        if updated_user:
            await call.message.edit_text(
                f"✅ @{updated_user.username or user_id} foydalanuvchisi ruxsat oldi (WORKER)."
            )
            try:
                await call.bot.send_message(
                    user_id,
                    "🎉 Sizning so'rovingiz tasdiqlandi! Endi sizga xodim menyusi ruxsat etildi.\n/start tugmasini bosing.",
                )
            except Exception:
                pass
        else:
...
async def cb_staff_edit_kpi(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Yangi KPI kiriting (so'm yoki donada maqsadli qiymat):", reply_markup=cancel_admin_inline_kb()
    )
...
async def cb_staff_revoke(call: types.CallbackQuery, container: Container):
    try:
        ...
        if updated_user:
            await call.message.edit_text(
                "🗑 Xodim faol ro'yxatdan o'chirildi. (Uni qaytarish uchun u /start bosishi kerak)"
            )
            try:
                await call.bot.send_message(
                    tg_id, "Profilingiz administrator tomonidan o'chirildi. Ruxsatingiz yopilgan."
                )
            except Exception:
                pass
        else:
...
async def cb_staff_export_excel(call: types.CallbackQuery, container: Container):
    try:
        ...
        await call.message.answer_document(
            document=file, caption=f"📥 @{user.username or tg_id} xodimi bo'yicha hisobot tayyor!"
        )
        await call.answer()
    except Exception:
...
async def cb_print_receipt(call: types.CallbackQuery, container: Container):
    ...
    try:
        ...
        if not printer_manager.has_connected_printer:
            await call.answer("⚠️ Printer hozirda ulanmagan! Printerni yoqing va qayta urinib ko'ring.", show_alert=True)
            return
        ...
        if success:
            ...
            if "Printer ulanmagan" in original_text:
                new_text = original_text.replace(
                    "⚠️ _Printer ulanmagan. Chekni qo'lda chop etish mumkin._", "🖨️ _Chek printerga yuborildi._"
                )
                try:
            ...
            await call.answer("❌ Chekni topib bo'lmadi yoki printerga yuborishda xatolik.", show_alert=True)
    except Exception:
...
async def cb_export_inventory_excel(call: types.CallbackQuery, container: Container):
    try:
        ...
        await call.message.answer_document(
            document=file, caption="📦 Ombordagi qoldiqlar bo'yicha joriy hisobot tayyor!"
        )
        await call.answer()
    except Exception:
...
async def receipt_select_cat(call: types.CallbackQuery, state: FSMContext, container: Container):
    try:
        ...
        await call.message.edit_text(
            "Qabul qilish uchun mahsulotni tanlang:", reply_markup=sell_product_list_kb(products)
        )
    except Exception:
...
async def receipt_select_product(call: types.CallbackQuery, state: FSMContext, container: Container):
    try:
        ...
        await call.message.edit_text(
            f"📦 Qabul qilish: <b>{product.name}</b>\nJoriy qoldiq: {product.quantity} dona.\n\nKirim miqdorini kiriting:",
            parse_mode="HTML",
            reply_markup=cancel_admin_inline_kb(),
        )
...
async def process_receipt_quantity(message: types.Message, state: FSMContext, container: Container, db_user: User):
    try:
        ...
        await message.answer(
            f"✅ <b>{data['product_name']}</b> mahsulotidan {qty} dona muvaffaqiyatli qabul qilindi.",
            parse_mode="HTML",
...
async def start_write_off(message: types.Message, state: FSMContext, container: Container):
    try:
        ...
        await message.answer(
            "Hisobdan chiqarish uchun kategoriyani tanlang:", reply_markup=worker_categories_kb(categories)
        )
    except Exception:
...
async def write_off_select_cat(call: types.CallbackQuery, state: FSMContext, container: Container):
    try:
        ...
        await call.message.edit_text(
            "Hisobdan chiqarish uchun mahsulotni tanlang:", reply_markup=sell_product_list_kb(available)
        )
    except Exception:
...
async def process_write_off_quantity(message: types.Message, state: FSMContext):
    ...
    if qty > data["max_qty"]:
        await message.answer(
            f"Ombordagidan ({data['max_qty']}) ko'prog'ini hisobdan chiqarib bo'lmaydi. Qaytadan kiriting:"
        )
        return
...
async def process_write_off_reason(message: types.Message, state: FSMContext, container: Container, db_user: User):
    try:
        ...
        data = await state.get_data()
        transaction_service: TransactionService = container.get("transaction_service")
        await transaction_service.create_write_off(
            user_id=db_user.id, product_id=data["product_id"], amount=data["quantity"], reason=reason
        )
        await message.answer(
            f"✅ <b>{data['product_name']}</b> mahsulotidan {data['quantity']} dona muvaffaqiyatli hisobdan chiqarildi, sabab: {reason}",
            parse_mode="HTML",
            reply_markup=main_admin_kb(),
        )
        await state.clear()
...
async def cb_bind_barcode(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Mahsulot shtrix-kodini yuboring (raqamlarni yozing yoki kamera yordamida skanerlang):",
        reply_markup=cancel_admin_inline_kb(),
    )
